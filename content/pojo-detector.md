---json
{
  "kind": "article",
  "crossPosts": [],
  "description": "Tons of things in JavaScript are objects of one sort or another. How can you differentiate between plain vanilla objects and others?",
...
(The `typeof` operator does get more specific for functions, at least. Functions _are still objects_, though.)
...
By this definition `{hello:'world'}` and `Object.create(null)` are both POJOs, but `new Date()`, `new Map()`, `/regex/`, _etc_ are not. That's usually what I'm trying to differentiate when I want to know if something is just a plain old object.

The following function checks if an entity matches that definition of a POJO, though note that the return type only accounts for _native_ types. You could extend the type to account for any specific custom classes you want to rule out -- as it stands, the function will return the correct _value_ for anything you pass into it, but it will incorrectly behave as a type guard for non-native class instances!
